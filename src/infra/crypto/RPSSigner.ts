...
export default class RPSSigner implements IRpsSigner {
  ...
  private pkcs12: forge.pkcs12.Pkcs12Pfx;
  private bags: any;
  private keyObj: forge.pki.rsa.PrivateKey;
  ...
  constructor(config: INfsConfigData) {
    ...
    this.pkcs12 = forge.pkcs12.pkcs12FromAsn1(
      forge.asn1.fromDer(this.pfx.toString("binary")),
      this.passphrase
    );
  ...
  private add(strAssinatura: string, segment: string, desc: string): string {
    ...
    if (this.config.debug) {
      console.log(
        `${desc}: +${
          segment.length
        } chars -> comprimento atual: ${len} (restam ${86 - len})`
      );
    }

    if (len > 86) {
      throw new ValidationException(
        `Comprimento excede 86 caracteres após inserir "${desc}": ${len}`
      );
    }
  ...
  public sign(rps: Rps): void {
    let strAssinatura = "";

    strAssinatura = this.add(
      strAssinatura,
      this.config.inscricaoPrestador,
      "CCM"
    );
    strAssinatura = this.add(
      strAssinatura,
      rps.ChaveRPS.SerieRPS.padEnd(5),
      "Série"
    );
    strAssinatura = this.add(
      strAssinatura,
      rps.ChaveRPS.NumeroRPS.padStart(12, "0"),
      "Número RPS"
    );
    strAssinatura = this.add(
      strAssinatura,
      rps.DataEmissao?.replaceAll("-", "") || "",
      "Data emissão"
    );
    strAssinatura = this.add(
      strAssinatura,
    ...
    strAssinatura = this.add(
      strAssinatura,
      String(rps.CodigoServico).padStart(5, "0"),
      "Código do serviço"
    );
    strAssinatura = this.add(
      strAssinatura,
      rps.TomadorRPS.CPFCNPJTomador.CPF ? "1" : "2",
      "Identificador pessoa"
    );
    strAssinatura = this.add(
      strAssinatura,
      String(
        rps.TomadorRPS.CPFCNPJTomador.CPF || rps.TomadorRPS.CPFCNPJTomador.CNPJ
      ).padStart(14, "0"),
      "CPF/CNPJ do tomador"
    );
    ...
    if (strAssinatura.length !== 86) {
      throw new ValidationException(
        `Comprimento final da string de assinatura do RPS inválido: ${strAssinatura.length}`
      );
    }
